...
class SaveTest extends CptTestCase {
	...
	public function success_simple() {
		...
		$testData = json_decode(self::getSimpleTestData());

		self::$settingsMock->shouldReceive('getImageSizes')->andReturn(self::getImageSizes());
		...
		$attachementMetadata = $this->get_attachement_metadata();
		...
		$request->shouldReceive('get_body')->andReturn(json_encode(['crop_thumbnails' => $testData]));
		$result = self::$cpt->saveThumbnail($request);
	...
	public function success_with_dynamic_width() {
		...
		$testData = json_decode(self::getDynamic2TestData());
	...
	public function success_with_dynamic_height() {
		...
		$testData = json_decode(self::getDynamic1TestData());
	...
	public function success_with_dynamic_zero() {
		/** SETUP **/
		$testData = json_decode(self::getDynamicZero());
		...
		$INPUT_wp_get_attachment_metadata = [];//this will be filled once the mock has run
		\WP_Mock::userFunction( 'wp_get_attachment_metadata',[
			'return' => function($id,$bool) use ($testData,$attachementMetadata,&$INPUT_wp_get_attachment_metadata) {
				$INPUT_wp_get_attachment_metadata = [$id,$bool];
				return $attachementMetadata;
			},
			//'times' => 1
		...
		\WP_Mock::userFunction('apply_filters', ['return' => function($filterName, $result,...$args) {
			error_log('TEST: '.$filterName);
			if($filterName === 'crop_thumbnails_do_crop') return 'Test 123';
			return $result;
		}]);
	...
	public static function get_attachement_metadata() {
		return [
			'width' => 3000,
			'height' => 2000,
